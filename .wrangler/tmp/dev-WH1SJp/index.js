...
// .wrangler/tmp/bundle-SyLDyd/checked-fetch.js
...
async function publishToCentrifugo(env, channel, data) {
  ...
  console.log(`\u{1F4E1} Worker: Publishing to Centrifugo on[${channel}]`, JSON.stringify(data).substring(0, 100) + "...");
  try {
      ...
      if (pathParts[0] === "documents") {
        ...
        if (method === "POST" && collection === "batch") {
          ...
          for (const op of operations) {
            ...
            const currentDoc = await env.DB.prepare("SELECT version FROM documents WHERE id = ?").bind(id).first();
...
VALUES(?, ?, ?, ?, ?, ?, (SELECT last_insert_rowid()))
        ...
        if (collection === "auth" && docId === "token" && method === "POST") {
          ...
          const token = await new SignJWT({
            sub: userId,
            user: userId
          }).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt().setExpirationTime("24h").sign(secretKey);
        ...
        if (method === "POST") {
          ...
          const { data, userId, parentPath, workspaceId } = await request.json();
          ...
          const docPath2 = parentPath ? `${parentPath} /${collection}/${id} ` : `${collection}/${id}`;
          ...
          try {
            ...
            ).bind(id, workspace, collection, docPath2, userId, JSON.stringify(data), version).run();
            const payload = {
              type: "CREATED",
              doc: { id, collection, path: docPath2, userId, data, version, workspaceId: workspace }
            };
            await publishToCentrifugo(env, `collection:${collectionPath.replace(/\//g, "_")}`, payload);
            await publishToCentrifugo(env, `path:${docPath2.replace(/\//g, "_")}`, payload);
            return Response.json({ id, path: docPath2, version, collection, workspaceId: workspace }, { status: 201, headers: corsHeaders });
          } catch (error) {
            return new Response(error.message, { status: 500, headers: corsHeaders });
          }
          ...
          return Response.json({
            changes: results.map((r) => ({ ...r, payload: JSON.parse(r.payload) })),
          ...
          const response = Response.json(results.map((r) => ({ ...r, data: JSON.parse(r.data) })), { headers: corsHeaders });
          ...
          const params = [collection, workspaceId];
          if (parentPath) {
          ...
          const { results } = await env.DB.prepare(query).bind(...params).all();
          return Response.json(results.map((r) => ({ ...r, data: JSON.parse(r.data) })), { headers: corsHeaders });
          ...
          return Response.json({ ...doc, data: JSON.parse(doc.data) }, { headers: corsHeaders });
        } else if (method === "PUT" && docId) {
          const { data, userId, workspaceId, parentPath, expectedVersion } = await request.json();
          const workspace = workspaceId || "default";
          const eventId = crypto.randomUUID();
          const doc = await env.DB.prepare("SELECT path, version FROM documents WHERE id = ?").bind(docId).first();
          if (doc) {
            if (expectedVersion !== void 0 && doc.version !== expectedVersion) {
            ...
            const docPath2 = parentPath ? `${parentPath}/${collection}/${docId}` : `${collection}/${docId}`;
            const collectionPath = parentPath ? `${parentPath}/${collection}` : `${collection}`;
            const eventRes = await env.DB.prepare(
              `INSERT INTO events (id, doc_id, workspace_id, event_type, payload) VALUES (?, ?, ?, 'INSERT', ?)`
            ).bind(eventId, docId, workspace, JSON.stringify(data)).run();
            ...
            await env.DB.prepare(
              `INSERT INTO documents (id, workspace_id, collection_name, path, user_id, data, version) VALUES (?, ?, ?, ?, ?, ?, ?)`
            ).bind(docId, workspace, collection, docPath2, userId, JSON.stringify(data), version).run();
            const payload = { type: "CREATED", doc: { id: docId, collection, path: docPath2, data, version, workspaceId: workspace } };
            await publishToCentrifugo(env, `collection:${collectionPath.replace(/\//g, "_")}`, payload);
            await publishToCentrifugo(env, `path:${docPath2.replace(/\//g, "_")}`, payload);
            return Response.json({ id: docId, path: docPath2, version }, { status: 201, headers: corsHeaders });
          }
        } else if (method === "PATCH" && docId) {
          const { data, userId, workspaceId, expectedVersion } = await request.json();
          const workspace = workspaceId || "default";
          const eventId = crypto.randomUUID();
          const doc = await env.DB.prepare("SELECT path, version FROM documents WHERE id = ?").bind(docId).first();
          if (!doc) return new Response("Not Found", { status: 404, headers: corsHeaders });
          ...
          const updatedDoc = await env.DB.prepare("SELECT * FROM documents WHERE id = ?").bind(docId).first();
          ...
          const { expectedVersion } = await request.json().catch(() => ({}));
          const doc = await env.DB.prepare("SELECT path, workspace_id, version FROM documents WHERE id = ?").bind(docId).first();
          if (doc) {
            if (expectedVersion !== void 0 && doc.version !== expectedVersion) {
              return new Response("Version Conflict", { status: 409, headers: corsHeaders });
            }
...
// .wrangler/tmp/bundle-SyLDyd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SyLDyd/middleware-loader.entry.ts
