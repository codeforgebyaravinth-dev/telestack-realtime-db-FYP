...
// .wrangler/tmp/bundle-BFFx2R/checked-fetch.js
...
var AensEngine = class {
  ...
  /**
   * AENS Algorithm: Adaptive Threshold Calculation
   * High velocity -> Wait longer to coalesce more
   * Low velocity -> Flush sooner for durability
   * @param {number} velocity
   * @param {number} predictability
   * @param {number} base_window
   * @returns {number}
   */
  static calculate_threshold(velocity, predictability, base_window) {
    const ret = wasm.aensengine_calculate_threshold(velocity, predictability, base_window);
    return ret;
  }
};
...
import wasmModule2 from "./922d3c7d78a552abfd8eb34f6ae5e87b78ace4b9-wasm_engine_bg.wasm";
var SecurityEngine = class {
  ...
  evaluate(op, path, auth, data) {
    ...
    return SecurityEvaluator.evaluate(ruleString, contextJson);
  }
};

// src/write-buffer.ts
import wasmModule3 from "./922d3c7d78a552abfd8eb34f6ae5e87b78ace4b9-wasm_engine_bg.wasm";
var WriteBuffer = class _WriteBuffer {
  constructor(env) {
    this.env = env;
    if (!_WriteBuffer.wasmInitPromise) {
      _WriteBuffer.wasmInitPromise = __wbg_init(wasmModule3).then(() => {
      });
    }
  }
  static {
  ...
  async queueWrite(op, ctx) {
    ...
    const adaptiveThreshold = AensEngine.calculate_threshold(velocity, 0.5, 500);
    if (ctx) {
      ctx.waitUntil(this.scheduleFlush(adaptiveThreshold));
    } else {
      await this.scheduleFlush(adaptiveThreshold);
    }
  }
  async scheduleFlush(threshold) {
    if (_WriteBuffer.buffer.length >= _WriteBuffer.bufferSize) {
      await this.flush();
    } else if (Date.now() - _WriteBuffer.lastFlush >= threshold) {
      await this.flush();
    }
  }
  async flush() {
      ...
      if (!existing) {
        ...
        if (write.type === "UPDATE" && existing.type === "UPDATE" && write.data && existing.data) {
          try {
            ...
            console.error("WASM Merge Error in Buffer:", e);
          }
        }
        map.set(key, write);
      }
...
// src/cache.ts
import wasmModule4 from "./922d3c7d78a552abfd8eb34f6ae5e87b78ace4b9-wasm_engine_bg.wasm";
...
var src_default = {
  ...
  async handleRequest(request, env, ctx) {
    ...
    try {
      ...
      const isCollectionOperation = pathSegments.length >= 1;
      let resolvedDbId = null;
      console.log(`[Request] ${method} ${url.pathname} (depth: ${depth}, collection: ${collection}, docId: ${docId})`);
    ...
    VALUES(?, ?, ?, 'admin', ?)
      ...
      if (url.pathname === "/documents/auth/token" && method === "POST") {
        const body = await request.json();
        ...
        const keyRow = await env.DB.prepare(
          "SELECT project_id FROM api_keys WHERE key_secret = ?"
        ).bind(apiKeyHeader).first();
        if (!keyRow) {
          return Response.json({ error: "Invalid API key" }, { status: 401, headers: corsHeaders });
        }
        const workspaceId2 = keyRow.project_id;
        const secret = env.TOKEN_SECRET || "my_32_char_secret_key_testing_123";
        const token = await new SignJWT({ sub: userId, user: userId, workspaceId: workspaceId2, pub: true }).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt().setExpirationTime("24h").sign(new TextEncoder().encode(secret));
        return Response.json({ token, workspaceId: workspaceId2 }, { headers: corsHeaders });
      }
      ...
      const dataSegments = pathSegments[0] === "documents" ? pathSegments.slice(1) : pathSegments;
      if (dataSegments.length > 0) {
        const isDocOperation = dataSegments.length > 0 && dataSegments.length % 2 === 0;
        const isCollectionOperation2 = dataSegments.length > 0 && dataSegments.length % 2 !== 0;
        const collection2 = isDocOperation ? dataSegments[dataSegments.length - 2] : dataSegments[dataSegments.length - 1];
        const docId2 = isDocOperation ? dataSegments[dataSegments.length - 1] : null;
        const parentPath2 = isDocOperation ? dataSegments.slice(0, -2).join("/") : dataSegments.slice(0, -1).join("/");
        const depth2 = dataSegments.length;
        console.log(`[Trace] dataSegments: ${JSON.stringify(dataSegments)}, op: ${isDocOperation ? "DOC" : isCollectionOperation2 ? "COLL" : "NONE"}, collection: ${collection2}, docId: ${docId2} `);
        ...
        if (collection2 === "internal" && docId2 === "collections" && method === "GET") {
        ...
        if (collection2 === "internal" && docId2 === "subcollections" && method === "GET") {
        ...
        if (collection2 === "internal" && docId2 === "reset" && method === "POST") {
          await env.DB.batch([
        ...
        if (method === "POST" && collection2 === "batch") {
    ...
    VALUES(?, ?, ?, ?, ?, ?, ?, ?, 0)
        ...
        if (pathSegments[0] === "documents" && pathSegments[1] === "auth" && pathSegments[2] === "token" && method === "POST") {
          ...
          try {
            const body = await request.json();
            const { data, userId, parentPath: parentPath3 = "" } = body;
            const id = crypto.randomUUID();
            const fullDocPath = parentPath3 ? `${parentPath3}/${collection2}/${id}` : `${collection2}/${id}`;
            ...
            if (noCache) {
              ...
              await gateway2.query(dbId, `INSERT INTO documents (path, id, workspace_id, collection_name, parent_path, depth, data, version, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(workspace_id, path) DO UPDATE SET data = excluded.data, version = excluded.version, updated_at = CURRENT_TIMESTAMP, deleted_at = NULL`, [fullDocPath, id, workspace, collection2, parentPath3 || "", depth2 + 1, JSON.stringify(data), docVersion, userId || "anonymous"]);
            } else {
            ...
            ctx.waitUntil(cache2.delete(`coll:${workspace}:${collection2}:*`));
            const payload = { type: "CREATED", doc: { id, path: fullDocPath, data, version: Date.now() } };
            ctx.waitUntil(publishToCentrifugo(env, `_ws_${workspace}_collection_${collection2.replace(/\//g, "_")}`, payload));
        ...
        } else if (method === "GET" && collection2 === "sync") {
        ...
        } else if (method === "GET" && collection2 === "query") {
        ...
        } else if (method === "GET" && isCollectionOperation2 && !docId2) {
          const cache2 = new PredictiveCache(env);
          const noCache = request.headers.get("Cache-Control") === "no-cache";
          const cacheKey = `coll:${workspace}:${collection2}:${url.searchParams.get("parentPath") || "root"}`;
          const dbId2 = resolvedDbId2 || "native";
          const responseData = await cache2.get(cacheKey, async () => {
            const res = await new D1Gateway(env).query(dbId2, "SELECT * FROM documents WHERE collection_name = ? AND workspace_id = ? AND deleted_at IS NULL", [collection2, workspace]);
        ...
        } else if (method === "GET" && docId2) {
          const fullDocPath = parentPath2 ? `${parentPath2}/${collection2}/${docId2}` : `${collection2}/${docId2}`;
          const cache2 = new PredictiveCache(env);
          const noCache = request.headers.get("Cache-Control") === "no-cache";
          const dbId2 = resolvedDbId2 || "native";
        ...
        } else if (method === "PUT" && docId2) {
          const body = await request.json();
          const { data, userId } = body;
          const ifMatch = request.headers.get("If-Match");
          const dbId2 = resolvedDbId2 || "native";
          const fullPath = parentPath2 ? `${parentPath2}/${collection2}/${docId2}` : `${collection2}/${docId2}`;
          if (!await security.canWrite(fullPath, auth2, data)) {
          ...
          const eventId = crypto.randomUUID();
          const eventRes = await gateway2.query(dbId2, `INSERT INTO events (id, doc_id, workspace_id, event_type, payload) VALUES (?, ?, ?, ?, ?)`, [eventId, docId2, workspace, "SET", JSON.stringify(data)]);
          const version = eventRes.meta.last_row_id;
          await gateway2.query(dbId2, `INSERT INTO documents (path, id, workspace_id, collection_name, parent_path, depth, data, version, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(workspace_id, path) DO UPDATE SET data = excluded.data, version = excluded.version, updated_at = CURRENT_TIMESTAMP, deleted_at = NULL`, [fullPath, docId2, workspace, collection2, parentPath2 || "", depth2 || 0, JSON.stringify(data), version, userId || "anonymous"]);
          const cache2 = new PredictiveCache(env);
          ctx.waitUntil(cache2.set(`doc:${workspace}:${fullPath}`, { id: docId2, path: fullPath, data, version }));
          ctx.waitUntil(publishToCentrifugo(env, `_ws_${workspace}_global_cache`, {
            type: "CACHE_WARM",
            key: `doc:${workspace}:${fullPath}`,
            data: { id: docId2, path: fullPath, data, version }
          }));
        ...
        } else if (method === "PATCH" && docId2) {
          const body = await request.json();
          const { data: patchData, userId } = body;
          const dbId2 = resolvedDbId2 || "native";
          const fullPath = parentPath2 ? `${parentPath2}/${collection2}/${docId2}` : `${collection2}/${docId2}`;
          if (!await security.canWrite(fullPath, auth2, patchData)) {
            return new Response("Permission Denied (Security Rules)", { status: 403, headers: corsHeaders });
          }
          const buffer = WriteBuffer.getInstance(env);
          ...
          const cache2 = new PredictiveCache(env);
          ctx.waitUntil(cache2.set(`doc:${workspace}:${fullPath}`, {
            id: docId2,
            path: fullPath,
            data: patchData,
            // Note: This is partial, Cache will merge or invalidate on next GET
            version: Date.now()
          }));
        ...
        await gateway.query(dbId, "UPDATE documents SET deleted_at = CURRENT_TIMESTAMP WHERE path = ? AND workspace_id = ?", [fullDocPath, workspace]);
        const cache2 = new PredictiveCache(env);
        ctx.waitUntil(cache2.delete(`doc:${workspace}:${fullDocPath}`));
        ctx.waitUntil(cache2.delete(`coll:${workspace}:${collection}:*`));
        ctx.waitUntil(publishToCentrifugo(env, `_ws_${workspace}_collection_${collection.replace(/\//g, "_")}`, { type: "DELETED", doc: { id: docId, path: fullDocPath } }));
...
// .wrangler/tmp/bundle-BFFx2R/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BFFx2R/middleware-loader.entry.ts
